...
func createShortcut(path, target string) error {
	...
	defer link.Release()
	if _, err = oleutil.PutProperty(link, "TargetPath", target); err != nil {
		return err
	}
	if _, err = oleutil.PutProperty(link, "WorkingDirectory", filepath.Dir(target)); err != nil {
		return err
	}
	if _, err = oleutil.PutProperty(link, "IconLocation", target+",0"); err != nil {
		return err
	}
