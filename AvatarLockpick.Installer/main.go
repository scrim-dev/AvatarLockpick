...
func main() {
	...
	installButton.OnTapped = func() {
		if !info.published {
			dialog.ShowConfirm("Pre-release build", fmt.Sprintf("Version %s does not have a GitHub Release. It is an unpublished pre-release. Continue?", info.version), func(ok bool) {
				if ok { install(w, status, progress, installButton, info, installDesktop.Checked) }
			}, w)
			return
		}
	...
	go func() {
		...
		message := "Latest version: " + info.version
		if local == "" {
			message += "\nAvatarLockpick is not installed."
			installButton.SetText("Install")
		} else if local == info.version {
...
func install(w fyne.Window, status *widget.Label, progress *widget.ProgressBar, button *widget.Button, info remoteInfo, desktop bool) {
	...
	go func() {
		err := downloadAndInstall(info.version, desktop, func(value float64) { progress.SetValue(value) })
		if err != nil {
...
func getRemoteInfo() (remoteInfo, error) {
	...
	if err != nil { return remoteInfo{}, fmt.Errorf("could not read version.txt: %w", err) }
	defer response.Body.Close()
	if response.StatusCode != http.StatusOK { return remoteInfo{}, fmt.Errorf("version.txt returned HTTP %d", response.StatusCode) }
	body, err := io.ReadAll(io.LimitReader(response.Body, 1024))
	if err != nil { return remoteInfo{}, err }
	version := strings.TrimSpace(string(body))
	if version == "" { return remoteInfo{}, errors.New("version.txt is empty") }

	info := remoteInfo{version: version}
	release, err := client.Get(releaseAPIBase + version)
	if err != nil { return info, nil } // GitHub availability must not prevent a package install.
	defer release.Body.Close()
	if release.StatusCode == http.StatusOK {
		var metadata struct { Prerelease bool `json:"prerelease"` }
...
func downloadAndInstall(version string, desktop bool, report func(float64)) error {
	...
	if err != nil { return fmt.Errorf("could not download package: %w", err) }
	defer response.Body.Close()
	if response.StatusCode != http.StatusOK { return fmt.Errorf("package returned HTTP %d", response.StatusCode) }
	...
	if err := extractZip(data, staging); err != nil { return fmt.Errorf("invalid package: %w", err) }
	...
	if os.Getenv("LOCALAPPDATA") == "" { return errors.New("LOCALAPPDATA is not available") }
	...
	if err := createShortcut(startMenuShortcut(), filepath.Join(target, "AvatarLockpick.exe")); err != nil { return err }
	if desktop {
		if err := createShortcut(filepath.Join(os.Getenv("USERPROFILE"), "Desktop", "AvatarLockpick.lnk"), filepath.Join(target, "AvatarLockpick.exe")); err != nil { return err }
	} else {
...
func extractZip(data []byte, destination string) error {
	...
	for _, file := range reader.File {
		...
		if !strings.HasPrefix(filepath.Clean(name)+func() string { if file.FileInfo().IsDir() { return string(os.PathSeparator) }; return "" }(), root) && filepath.Clean(name) != strings.TrimSuffix(root, string(os.PathSeparator)) { return errors.New("zip contains an unsafe path") }
		if file.FileInfo().IsDir() { if err := os.MkdirAll(name, 0755); err != nil { return err }; continue }
		if err := os.MkdirAll(filepath.Dir(name), 0755); err != nil { return err }
		in, err := file.Open(); if err != nil { return err }
		out, err := os.OpenFile(name, os.O_CREATE|os.O_TRUNC|os.O_WRONLY, file.Mode()); if err != nil { in.Close(); return err }
		_, copyErr := io.Copy(out, in); out.Close(); in.Close()
...
func setStatus(label *widget.Label, value string) { label.SetText(value) }
